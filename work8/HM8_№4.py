...
class Printer(Technics):
    ...
    def __str__(self):
        return f"Printer {self.model} s/n {self.serial_no}"
...
class Warehouse:
    def __init__(self, max_volum):
        self.max_volume = max_volum
        self.storage = {"scanners":set(), "printers":set(), "xeroxes":set()}
        self.add_mapper = {Scanner: "scanners", Printer: "printers", Xerox: "xeroxes"}
        self.total = 0

    def get_several_tech_to_warehouse(self, num: int, tech_list):
        if type(num) != int:
            raise ValueError("Невалидное количество технгики! Введите число!")
...
warehouse.get_several_tech_to_warehouse(3, tech_list)
